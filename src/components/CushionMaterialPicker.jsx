// src/components/CushionMaterialPicker.jsx
import { useMemo, useState } from "react";
import { data } from "../config/data";

export default function CushionMaterialPicker({ visible }) {
  const [hover, setHover] = useState(null);

  const cushionMaterials = useMemo(() => {
    const mats = data.models.Sofa.materialsLods || [];
    return mats.filter((m) =>
      m.materialUsingCategories?.categoriesNames?.includes("Cushion Type")
    );
  }, []);
      ...
      <div className="px-8 py-6">
        <div className="flex items-center justify-center gap-6">
          {cushionMaterials.map((mat) => {
            ...
            const imageUrl = mat.materialUsingCategories?.materialImagePath || "";
            ...
            return (
              ...
              >
                <button
                  onClick={() => {
                    console.log(`🎨 Selected: ${mat.materialName}`);
                    window.dispatchEvent(
                      new CustomEvent("material-change", {
                        detail: { category: "Cushion Type", materialId: mat.id },
                      })
                    );
                  }}
                ...
                >
                  ...
                  <div
                    className={`
                      ...
                      border-4 
                      ${isHovered ? 'border-white scale-120' : 'border-white/70 scale-100'}
                      transition-all duration-300
                      pointer-events-none
                    `}
                    style={{
