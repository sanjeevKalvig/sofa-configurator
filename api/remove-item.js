...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!cartId || !itemId) return res.status(400).json({ error: 'Missing parameters' });
    ...
    const upstream = await fetch(upstreamUrl, { method: 'DELETE', headers: { ...headers, accept: 'application/json' } });
